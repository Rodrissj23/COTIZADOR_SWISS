...
const displayModality = modality => modality === DESREGULADO ? DESREGULADO_LABEL : modality;
...
const isPartialPlan = name => ['AMBU1','AMBU2','INTER1'].includes(name);

function getTariffFor(planName, modality, zone){
  // Tierra del Fuego: si se cotiza Monotributo usa la base Directo y conserva su bonificación comercial.
...
function discountForMember(age, role, modality, specialDiscount, zone){
  const candidates = [];
  if (Number(age) <= 25) candidates.push({percent:50,label:'Hasta 25 años'});
...
function adultListPrice(tariff, planName, age){
  const key = tariff?.bands?.find(b => Number(age) <= b.max)?.key;
  return key ? tariff?.adult?.[planName]?.[key] ?? null : null;
}

function contributionForClient(client){
  if (client.modality !== DESREGULADO) return {receiptContribution:0,baseCalculated:0,baseContribution:0,capApplied:false,aporteComputable:0};
  const receiptContribution = Math.max(0, Number(client.receiptContribution) || 0);
  // La planilla trabaja la base en pesos enteros antes de aplicar 9% x 0,85.
...
function familyQuote(plan){
  ...
  members.push({role:'Titular',age:c.age,listPrice:titularList,...discountForMember(c.age,'Titular',c.modality,c.specialDiscount,c.zone)});
  ...
  if (hasPartner(c.familyType)){
    ...
    members.push({role:'Pareja',age:c.partnerAge,listPrice:partnerList,...discountForMember(c.partnerAge,'Pareja',c.modality,c.specialDiscount,c.zone)});
  }

  if (hasChildren(c.familyType)){
    const ages = Array.isArray(c.childrenAges) ? c.childrenAges.slice(0,c.children) : [];
    if (ages.length !== c.children || ages.some(age => !Number.isFinite(age) || age < 0 || age > 25)) {
    ...
    if (isPartialPlan(plan.name) && c.children > 1) return {status:'consult',reason:'Para este plan, más de un hijo requiere confirmación comercial.'};

    let firstChildTariffUsed = false;
    ages.forEach((childAge,index)=>{
      ...
      members.push({
        ...
        ...discountForMember(childAge,'Hijo',c.modality,c.specialDiscount,c.zone),
        tariffRole:mustUseAdditional?'Hijo adicional':'1er hijo'
      });
    });
    if (members.some(m=>m.listPrice == null)) return {status:'consult',reason:'La tabla no informa tarifa de hijo adicional para este plan.'};
  }
  ...
  const contribution = contributionForClient(c);
...
function renderChildAgeFields(count){
  ...
  if (!wrap) return;
  const previous = $$('.child-age',wrap).map(input=>input.value);
  wrap.innerHTML = Array.from({length:count},(_,i)=>`<label>Edad hijo ${i+1}<input class="child-age" type="number" min="0" max="25" value="${previous[i] ?? Math.min(21,10+i)}" required></label>`).join('');
}
...
function updateCompositionFields(){
  ...
  const count=childrenEnabled?Math.max(1,Number($('#children').value)||1):0;
...
function syncCase(){
  ...
  const familyType=$('#familyType').value;
  const children=hasChildren(familyType)?Math.max(1,Number($('#children').value)||1):0;
...
$('#quoteForm').addEventListener('input',event=>{
  if (event.target.id==='children') updateCompositionFields();
  syncCase();
...
$('#children').addEventListener('input',()=>renderChildAgeFields(Math.max(1,Number($('#children').value)||1)));
...
function renderPlans(){
  ...
  $('#plansGrid').innerHTML=plans.map(({plan:p,quote})=>{
    ...
    return `<article class="plan-card">
      ...
      <div class="plan-price"><small>${isOk?'Valor mensual final':'Validación necesaria'}</small><strong>${isOk?money(quote.finalPrice):'Consultar'}</strong><small>${isOk?quoteSummary(quote):quote.reason}</small></div>
      <button class="button button--red" data-plan="${p.name}">${isOk?'Elegir plan':'Consultar'} <span>→</span></button>
    </article>`;
  }).join('');
...
function footer(page){return `<div class="quote-footer"><span>Tarifario ${TARIFF_LABEL} · propuesta válida por ${QUOTE_VALIDITY_HOURS} hs</span><span>${page} / 5 · Grupo Zeroka</span></div>`}

function memberBreakdown(quote){
  if (quote.status!=='ok') return `<p>${quote.reason||'Requiere validación comercial.'}</p>`;
  const rows=quote.members.map(m=>`<div class="coverage-row"><span><b>${m.role} · ${m.age} años</b><br><small>${m.tariffRole?`${m.tariffRole} · `:''}Lista ${money(m.listPrice)} · ${m.percent}% ${m.label}</small></span><span>${money(m.finalPrice)}</span></div>`).join('');
  const aporte=quote.aporteComputable>0?`<div class="coverage-row coverage-row--aporte"><span><b>Aportes computables</b><br><small>Descuento aplicado por recibo de sueldo</small></span><span>− ${money(quote.aporteComputable)}</span></div>`:'';
  return rows+aporte;
}
...
function benefitsHtml(planName){
  ...
  return `<div class="technical-head"><span>${benefit.system}</span><span>${benefit.line}</span><span>Fuente ${benefit.source}</span></div><div class="technical-grid">${benefit.highlights.map((item,i)=>`<article><b>${String(i+1).padStart(2,'0')}</b><p>${item}</p></article>`).join('')}</div>`;
}
...
function buildQuote(){
  ...
  <section class="quote-page quote-cover"><div class="quote-circle"></div><div class="quote-logo">${brand(true)}</div><div class="quote-cover-copy"><p>PROPUESTA PERSONALIZADA · ${TARIFF_LABEL.toUpperCase()}</p><h1>Hola, ${c.name.split(' ')[0]}.<br>Tu salud merece una buena decisión.</h1><p>${clientMeta}</p><p class="quote-validity">Emitida ${dates.issued} · válida hasta ${dates.valid}</p></div><div class="cover-card"><div><small>PLAN ELEGIDO</small><strong>${displayPlanName(p.name)}</strong><span>${compositionLabel(c)}</span></div><div><small>VALOR MENSUAL FINAL</small><strong>${money(quote.finalPrice)}</strong><span>${quoteSummary(quote)}</span></div></div></section>
  <section class="quote-page quote-network"><div class="network-photo"><img src="assets/images/login-doctor.jpg" alt="Atención personalizada Swiss Medical"></div><div class="network-copy"><p class="eyebrow">UNA RED PARA CUIDARTE</p><h2>Más cerca cuando lo necesitás.</h2><p>La propuesta combina el plan seleccionado con la cartilla y condiciones vigentes para la zona informada.</p><div class="quote-metrics quote-metrics--editorial"><div class="metric"><strong>Cartilla</strong><span>según plan y zona</span></div><div class="metric"><strong>Atención</strong><span>ambulatoria e internación según alcance</span></div><div class="metric"><strong>Respaldo</strong><span>Swiss Medical</span></div><div class="metric"><strong>Asesoría</strong><span>Grupo Zeroka</span></div></div></div><div class="network-note"><b>La disponibilidad de prestadores se confirma al momento de la contratación.</b><span>Los alcances médicos se rigen por la documentación oficial vigente del plan.</span></div>${footer(2)}</section>
  <section class="quote-page quote-benefits"><aside class="benefits-aside"><p>PLAN ELEGIDO</p><strong>${displayPlanName(p.name)}</strong><span>${clientMeta}</span><div class="benefits-aside-mark">SM</div><small>Valores según tarifario ${TARIFF_LABEL}.</small></aside><div class="benefits-main"><p class="eyebrow">DETALLE ECONÓMICO</p><h2>Precio claro, integrante por integrante.</h2><p class="benefits-intro">Cada integrante utiliza su tarifa correspondiente y una sola bonificación: siempre la mayor aplicable.</p><div class="coverage-list">${memberBreakdown(quote)}</div><div class="benefits-callout"><span>$</span><p><b>Resumen:</b> lista ${money(quote.listPrice)} · bonificaciones ${money(quote.discount)}${quote.aporteComputable>0?` · aportes ${money(quote.aporteComputable)}`:''} · <b>final ${money(quote.finalPrice)}</b>.</p></div></div>${footer(3)}</section>
  <section class="quote-page quote-technical"><div class="technical-page-inner"><p class="eyebrow">ALCANCE DEL PLAN</p><h2>${displayPlanName(p.name)}</h2><p class="technical-intro">Resumen comercial de las prestaciones más relevantes. Los topes, cargos, reintegros, exclusiones y condiciones completas se rigen por la documentación oficial de Swiss Medical.</p>${benefitsHtml(p.name)}</div>${footer(4)}</section>
  <section class="quote-page quote-close"><div class="close-shape one"></div><div class="close-shape two"></div><div style="z-index:1">${brand(true)}<h2>Estamos para acompañarte.</h2><p>Esta propuesta es válida por ${QUOTE_VALIDITY_HOURS} horas desde su emisión.</p><small>Propuesta preparada por Grupo Zeroka · Tarifario ${TARIFF_LABEL}</small></div>${footer(5)}</section>`;
}
...
function pdfFooter(doc,page){doc.setFontSize(8);doc.setTextColor(119,109,110);doc.text(`Tarifario ${TARIFF_LABEL} · válida ${QUOTE_VALIDITY_HOURS} hs`,16,289);doc.text(`${page} / 5 · Grupo Zeroka`,166,289);doc.setTextColor(33,25,26)}
...
function downloadQuotePDF(){
  ...
  let y=51;y=pdfText(doc,`Hola, ${c.name.split(' ')[0]}.`,16,y,178,27,'bold');y+=5;
  ...
  pdfText(doc,`Emitida: ${dates.issued}`,16,244,178,9);pdfText(doc,`Válida hasta: ${dates.valid}`,16,251,178,9,'bold');pdfFooter(doc,1);
  ...
  pdfText(doc,'La disponibilidad de prestadores y las condiciones finales se confirman al momento de la contratación.',16,235,178,9);pdfFooter(doc,2);

  addPage();pdfHeader(doc,'Detalle económico',`${displayPlanName(p.name)} · ${TARIFF_LABEL}`);y=47;
  quote.members.forEach(m=>{doc.setDrawColor(231,221,221);doc.line(16,y+4,194,y+4);y=pdfText(doc,`${m.role} · ${m.age} años`,16,y+11,80,11,'bold');pdfText(doc,`${money(m.listPrice)} lista · ${m.percent}% ${m.label}`,16,y+2,115,9);doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text(money(m.finalPrice),194,y-8,{align:'right'});y+=14;});
  if(quote.aporteComputable>0){doc.setDrawColor(231,221,221);doc.line(16,y,194,y);y=pdfText(doc,'Aportes computables',16,y+10,110,11,'bold');doc.text(`- ${money(quote.aporteComputable)}`,194,y-5,{align:'right'});y+=8;}
  doc.setFillColor(247,245,244);doc.roundedRect(16,225,178,37,5,5,'F');pdfText(doc,`Lista: ${money(quote.listPrice)} · Bonificaciones: ${money(quote.discount)}${quote.aporteComputable>0?` · Aportes: ${money(quote.aporteComputable)}`:''}`,22,239,165,9);pdfText(doc,`TOTAL FINAL: ${money(quote.finalPrice)}`,22,252,165,16,'bold');pdfFooter(doc,3);
  ...
  (benefit?.highlights||['Consultar documentación oficial del plan.']).forEach((item,i)=>{doc.setFillColor(249,230,229);doc.circle(20,y+7,3,'F');y=pdfText(doc,item,28,y+10,160,10);y+=6;if(y>270)return;});pdfFooter(doc,4);

  addPage();doc.setFillColor(109,13,22);doc.rect(0,0,210,297,'F');doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(25);doc.text('SWISS MEDICAL',105,95,{align:'center'});doc.setFontSize(30);doc.text('Estamos para',105,139,{align:'center'});doc.text('acompañarte.',105,153,{align:'center'});doc.setFont('helvetica','normal');doc.setFontSize(12);doc.text(`Propuesta válida por ${QUOTE_VALIDITY_HOURS} horas`,105,178,{align:'center'});doc.setFontSize(9);doc.text(`Grupo Zeroka · Tarifario ${TARIFF_LABEL}`,105,244,{align:'center'});

  const safeName=(c.name||'Cliente').replace(/[\\/:*?"<>|]/g,'').trim();
