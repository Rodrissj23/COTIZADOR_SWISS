...
function renderPlans(){
  ...
  $('#plansGrid').innerHTML=plans.map((p,i)=>{const price=priceFor(p);return `<article class="plan-card ${p.name==='SMG30'?'featured':''}"><div class="plan-top"><h3>${p.name}</h3><span class="tag">${p.tag}</span></div><p class="plan-family">Familia ${p.family}</p><div class="plan-price"><small>Valor mensual</small><strong>${money(price)}</strong><small>${price==null?'Requiere validación comercial':'Importe por composición seleccionada'}</small></div><button class="button ${p.name==='SMG30'?'button--white':'button--red'}" data-plan="${p.name}">${price==null?'Consultar':'Elegir plan'} <span>→</span></button></article>`}).join('');
  $$('[data-plan]').forEach(b=>b.addEventListener('click',()=>selectPlan(b.dataset.plan)));
}
function selectPlan(name){state.plan=window.SWISS_PLANS.find(p=>p.name===name);const price=priceFor(state.plan);$('#selectedName').textContent=name;$('.selected-price strong').textContent=money(price);$('.selected-price small').textContent=price==null?'Requiere validación comercial':'Valor mensual por composición';$('#selectedBar').hidden=false;$('#selectedBar').scrollIntoView({behavior:'smooth',block:'end'});}
