...
await test('Desregulado $20.000 deja SMG30 en $241.124,97',async()=>{const {context,page}=await newPage();await openIndex(page);await chooseModality(page,'Desregulado');await page.locator('#receiptContribution').fill('20000');await submit(page);const c=await card(page,'SMG30');assert((await c.textContent()).includes('241.124,97'),`precio inesperado: ${await c.textContent()}`);await context.close()});
...
await test('Cotización individual usa 4 páginas base y una sola hoja técnica',async()=>{
  ...
  assert(await page.locator('#quotePages .quote-page').count()===4,'la propuesta individual debería tener 4 páginas');assert(await page.locator('.ref-cover').count()===1,'falta portada de referencia');assert(await page.locator('.ref-network').count()===1,'falta página institucional');assert(await page.locator('.ref-network img[src="assets/images/swiss-network-reference.svg"]').count()===1,'la institucional debe usar la maqueta fija de referencia');assert(await page.locator('.ref-summary-svg').count()===1,'falta hoja económica SVG');assert(await page.locator('.ref-technical').count()===1,'el alcance debe concentrarse en una sola página');await context.close();
});
