...
async function card(page,name){return page.locator('.plan-card').filter({has:page.locator('h3',{hasText:name})}).first()}
...
await test('Planes parciales se ocultan a los 18 y aparecen desde los 20',async()=>{const {context,page}=await newPage();await openIndex(page);await page.locator('#age').fill('18');await submit(page);assert(await page.locator('.plan-card h3',{hasText:'AMBU1'}).count()===0,'AMBU1 no debería aparecer a los 18');await page.locator('[data-scroll="#cotizador"]').last().click();await page.locator('#age').fill('20');await submit(page);assert(await page.locator('.plan-card h3',{hasText:'AMBU1'}).count()===1,'AMBU1 debería aparecer a los 20');await context.close()});
...
await test('Familia grande pagina el detalle por integrante y no pierde integrantes',async()=>{
  ...
  assert(await page.locator('#quotePages .quote-page').count()===5,'7 hijos deberían generar 5 páginas de propuesta');const economic=page.locator('#quotePages .ref-family-detail');assert(await economic.count()===2,'detalle por integrante debería ocupar 2 páginas');const text=(await economic.allTextContents()).join(' ');for(let i=1;i<=7;i++)assert(text.includes(`Hijo ${i}`),`falta Hijo ${i} en la propuesta`);await context.close();
});

await test('Cotización individual usa 3 páginas comerciales y la institucional original',async()=>{
  ...
  assert(await page.locator('#quotePages .quote-page').count()===3,'la propuesta individual debería tener 3 páginas comerciales');assert(await page.locator('.ref-cover').count()===1,'falta portada de referencia');assert(await page.locator('.ref-network').count()===1,'falta página institucional');assert(await page.locator('.ref-network img[src="assets/images/swiss-network-original.jpg"]').count()===1,'la institucional debe usar la captura original');assert(await page.locator('.ref-summary-svg').count()===1,'falta hoja económica SVG');assert(await page.locator('.ref-technical').count()===0,'el preview no debe duplicar alcance técnico reconstruido');assert((await page.locator('.dialog-toolbar small').textContent()).includes('cuando está disponible'),'el toolbar debe aclarar el alcance oficial');await context.close();
});

await test('Hoja económica mantiene todos los renglones y valores dentro del SVG',async()=>{
  const {context,page}=await newPage();await openIndex(page);await submit(page);const c=await card(page,'SMG30');await c.locator('[data-plan]').click();await page.locator('#openQuote').click();
  const svg=page.locator('.ref-summary-svg');assert(await svg.isVisible(),'SVG económico no visible');const text=await svg.textContent();
  for(const label of ['Grupo familiar','Zona','Valor detalle','Fliar a cargo','Aportes a descontar','Descuento promocional','Descuento multiproducto','IVA','Total'])assert(text.includes(label),`falta renglón ${label}`);
  const box=await svg.boundingBox();assert(box&&Math.round(box.width)===794&&Math.round(box.height)===1123,`dimensiones inesperadas ${JSON.stringify(box)}`);await context.close();
});

await test('Hoja económica Desregulado refleja aporte y total exactos',async()=>{
  const {context,page}=await newPage();await openIndex(page);await chooseModality(page,'Desregulado');await page.locator('#receiptContribution').fill('20000');await submit(page);const c=await card(page,'SMG30');await c.locator('[data-plan]').click();await page.locator('#openQuote').click();
  const text=await page.locator('.ref-summary-svg').textContent();assert(text.includes('$ 51.000')||text.includes('$ 51.000')||text.includes('$51.000'),`aporte no visible: ${text}`);assert(text.includes('$ 241.125')||text.includes('$ 241.125')||text.includes('$241.125'),`total no visible: ${text}`);await context.close();
});

await test('Descarga PDF directa con nombre correcto y contenido no vacío aunque falte alcance oficial',async()=>{const {context,page,errors}=await newPage();await openIndex(page);await submit(page);const c=await card(page,'SMG30');await c.locator('[data-plan]').click();await page.locator('#openQuote').click();const downloadPromise=page.waitForEvent('download');await page.locator('#printQuote').click();const download=await downloadPromise;assert(download.suggestedFilename()==='Cotizacion Swiss Medical (QA Cliente).pdf',`nombre recibido: ${download.suggestedFilename()}`);const path=await download.path();assert(path&&fs.statSync(path).size>10000,`PDF demasiado pequeño: ${path?fs.statSync(path).size:0}`);assert(errors.length===0,errors.join(' | '));await context.close()});
await test('Tierra del Fuego + Monotributo no ofrece planes integrales sin tabla',async()=>{const {context,page}=await newPage();await openIndex(page);await page.locator('#zone').selectOption({label:'Tierra del Fuego'});await chooseModality(page,'Monotributo');await submit(page);const names=await page.locator('.plan-card h3').allTextContents();assert(names.every(name=>['AMBU1','AMBU2','INTER1'].includes(name.trim())),`planes inesperados: ${names.join(', ')}`);assert(names.length===3,`esperaba 3 parciales, recibí ${names.length}`);await context.close()});
...
await test('Login desktop y mobile no generan overflow horizontal',async()=>{for(const viewport of [{width:1440,height:900},{width:390,height:844}]){const {context,page,errors}=await newPage(viewport);await page.goto(`${BASE}/login.html`,{waitUntil:'networkidle'});const d=await page.evaluate(()=>({scroll:document.documentElement.scrollWidth,client:document.documentElement.clientWidth}));assert(d.scroll<=d.client+2,`login overflow ${viewport.width}px: ${JSON.stringify(d)}`);assert(await page.locator('#loginForm').isVisible(),'form login no visible');assert(errors.length===0,errors.join(' | '));await context.close()}});
