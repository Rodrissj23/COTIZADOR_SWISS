...
await test('Familia grande pagina el detalle por integrante y conserva el resumen técnico',async()=>{
  ...
  assert(await page.locator('#quotePages .quote-page').count()===6,'7 hijos deberían generar 6 páginas de propuesta');const economic=page.locator('#quotePages .ref-family-detail');assert(await economic.count()===2,'detalle por integrante debería ocupar 2 páginas');const text=(await economic.allTextContents()).join(' ');for(let i=1;i<=7;i++)assert(text.includes(`Hijo ${i}`),`falta Hijo ${i} en la propuesta`);assert(await page.locator('.ref-technical').count()===1,'debe conservarse el resumen técnico');await context.close();
});

await test('Cotización individual usa 4 páginas y conserva resumen técnico',async()=>{
  ...
  assert(await page.locator('#quotePages .quote-page').count()===4,'la propuesta individual debería tener 4 páginas');assert(await page.locator('.ref-cover').count()===1,'falta portada de referencia');assert(await page.locator('.ref-network').count()===1,'falta página institucional');assert(await page.locator('.ref-network img[src="assets/images/swiss-network-original.jpg"]').count()===1,'la institucional debe usar la captura original');assert(await page.locator('.ref-summary-svg').count()===1,'falta hoja económica SVG');assert(await page.locator('.ref-technical').count()===1,'debe mantenerse una hoja técnica resumida');assert((await page.locator('.dialog-toolbar small').textContent()).includes('resumen técnico incluido'),'el toolbar debe aclarar el resumen técnico');await context.close();
});

await test('Todos los planes tienen contenido técnico visible en la propuesta',async()=>{
  ...
  for(const name of ['S1','S2','SMG02','SMG20','SMG30','SMG40','SMG50','SMG60','SMG70','SPORT S','SPORT','SPORT+','AMBU1','AMBU2','INTER1']){
    const c=await card(page,name);assert(await c.count()===1,`no se encontró ${name}`);await c.locator('[data-plan]').click();await page.locator('#openQuote').click();await page.waitForTimeout(10);const tech=page.locator('.ref-technical');assert(await tech.count()>=1,`${name} quedó sin resumen técnico`);assert((await tech.allTextContents()).join(' ').trim().length>80,`${name} tiene resumen técnico vacío`);await page.locator('#closeQuote').click();
  }
  await context.close();
});
